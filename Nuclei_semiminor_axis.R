...
oregonr <- subset(data, Genotype == "OregonR")$Ellipse_Minor
prtp <- subset(data, Genotype == "prtp")$Ellipse_Minor
...
data_combined <- data.frame(
  ...
  Ellipse_Minor = c(oregonr, prtp)
)

#OregonR and prtp boxplot
ggplot(data_combined, aes(x = Genotype, y = Ellipse_Minor, fill = Genotype)) +
  ...
  labs(x = "", y = expression("Nuclei semiminor axis (µm)")) + 
  scale_y_continuous(limits = c(10, 30), breaks = seq(0, 25, by = 5)) + 
